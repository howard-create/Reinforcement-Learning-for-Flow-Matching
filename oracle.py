...
LightningModel.on_load_checkpoint = patched_on_load_checkpoint

base_path = '/home/ubuntu'


def get_gosai_oracle(mode='train'): 
    if mode == 'train':  #mdlm/outputs_gosai/lightning_logs/reward_oracle_ft.ckpt  #'mdlm/gosai_data/binary_atac_cell_lines.ckpt'
        ...
        model_load = LightningModel.load_from_checkpoint(os.path.join(base_path, 'mdlm/gosai_data/binary_atac_cell_lines.ckpt'), map_location='cuda')
    else:
        raise ValueError
...
def subset_for_eval(config = None, n=5000, seed=0):
    train_set = dataloader_gosai.get_datasets_gosai(config)
    np.random.seed(seed)
...
def cal_highexp_kmers(k=3, return_clss=False, config = None):
    train_set = dataloader_gosai.get_datasets_gosai(config)
