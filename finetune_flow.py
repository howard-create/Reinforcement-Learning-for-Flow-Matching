    ...
    for epoch_num in range(cfg.num_epochs):
        ...
        for _step in range(cfg.num_accum_steps):
            ...
            for random_t in range(cfg.total_num_steps):
                    ...
                    #continue
                if cfg.adv == "gae":
                    if random_t == cfg.total_num_steps - 1:
                        x_now = last_x_list[random_t]
                        condt_now = condt_list[random_t]
                        logits_now = logits_list[random_t]
                        V_t = score(x_now, new_model, new_model_y, B, L, cfg.gumbel_temp)
                        V_next = 0
                        sample_t = diff_sample(logits_now[:,:,:-1], cfg)
                    else:
                        x_now = last_x_list[random_t]
                        x_next = last_x_list[random_t+1]
                        condt_now = condt_list[random_t]
                        logits_now = logits_list[random_t]
                        V_t = score(x_now, new_model, new_model_y, B, L, cfg.gumbel_temp)
                        V_next = score(x_next, new_model, new_model_y, B, L, cfg.gumbel_temp)
                        sample_t = diff_sample(logits_now[:,:,:-1], cfg)       #這裡應該是根據現有logits做sample，use differentiable type
                    preds_t = new_model_y(sample_t.float().transpose(1, 2))
                    reward_t = preds_t[:,0]  

                    with torch.no_grad():
                        delta = reward_t + gamma * V_next - V_t
                        lastgaelam = delta + gamma * lam * lastgaelam
                        lastgaelam_loss = lastgaelam.view(B, 1, 1)
                        lastgaelam_loss = lastgaelam_loss.expand(B, L, 4)
                        #print(lastgaelam.shape) #[8,1]
                if cfg.adv == "mse":
                ...
                loss = loss + log_p_x0 * lastgaelam_loss
...
if __name__ == "__main__":
    ...
    ckpt_path = os.path.join(base_path, 'mdlm/reward_bp_results_final/alpha0.001_accum4_bsz1_truncate10_temp1.0_clip1.0_test_20260223_135052/model_999.ckpt')  
