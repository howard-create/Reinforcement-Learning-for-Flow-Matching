...
def fine_tune(new_model, new_model_y, new_model_y_eval, old_model, cfg, log_path, save_path, train_loader, val_loader=None, eps=1e-5):
    ...
    L = 200
    #B 改 64
    #seqs = F.one_hot(seqs, 4).float()
    #seqs_diff = diff_sample(seqs, cfg)
    #baseline = new_model_y(seqs_diff.float().transpose(1,2))
    #baseline = baseline[:,0]
    #baseline = baseline.mean().detach
    #baseline = baseline.view(B, 1)
    mask_id = 4
    gamma =  0.5
    lam = 1.0
    lastgaelam = 0.0
    ...
    for epoch_num in range(cfg.num_epochs):
        ...
        for _step in range(cfg.num_accum_steps):
            ...
            #lam_raw = reward - reward.mean()                       # [B]
            #adv = (lam_raw - lam_raw.mean()) / (lam_raw.std(unbiased=False) + 1e-8)  # [B]
            ...
            lam = adv.view(B, 1, 1)                                # [B,1,1]
            #print(reward)
            #print(baseline_r)
            #print(lam)
            #lam = advantage
            lam_all.append(lam.detach().cpu())
            #lam = lam.view(B, 1, 1)
        ...
        if cfg.name != 'debug':
            wandb.log({
                ...
                "lam_distribution": wandb.Histogram(lam.detach().cpu().numpy())
            })
...
if __name__ == "__main__":
    base_path = "/home/ubuntu"  #mdlm/reward_bp_results_final/alpha0.001_accum4_bsz1_truncate10_temp1.0_clip1.0_test_20260302_163800/model_999.ckpt
    ckpt_path = os.path.join(base_path, 'mdlm/reward_bp_results_final/alpha0.001_accum4_bsz1_truncate10_temp1.0_clip1.0_test_20260302_163800/model_1749.ckpt')   
