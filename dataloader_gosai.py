...
def index_to_dna(seq):
    # seq: torch tensor (cpu/cuda) or numpy array
    if torch.is_tensor(seq):
      seq = seq.detach().to("cpu").numpy()
    if seq.ndim == 1:
      return "".join(lookup_array[seq])
    else:
      return ["".join(lookup_array[s]) for s in seq]
