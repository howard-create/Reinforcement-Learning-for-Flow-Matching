...
def fine_tune(new_model, new_model_y, new_model_y_eval, old_model, cfg, log_path, save_path, train_loader, val_loader=None, eps=1e-5):
    ...
    for epoch_num in range(cfg.num_epochs):
        ...
        for _step in range(cfg.num_accum_steps):
            ...
            '''
            for mse_t in range(cfg.total_num_steps):
                    x_now = last_x_list[mse_t]
                    condt_now = condt_list[mse_t]
                    logits_now = new_model(x_now, condt_now)
                    sample_t = diff_sample(logits_now[:,:,:-1], cfg)    
                    preds_t = new_model_y(sample_t.float().transpose(1, 2))
                    reward_t = preds_t[:,0]  # B = 8
                    reward_list.append(reward_t)
                    reward_t = reward_t.view(mini_number, seg) #mini_number = 2, total B = 8
                    baseline_r = reward_t.mean(dim = 1 )
                    #print(baseline_r.shape)
                    baseline_r = baseline_r.repeat_interleave(seg)
                    #print(baseline_r.shape)
                    baseline_r = baseline_r.view(B, 1)
                    baseline_r_list.append(baseline_r)
                    #if n == B/mini:
                        #torch.stack(mini_reward_list)
            '''

            reward = reward.view(mini_number, seg) #mini_number = 2, total B = 8
            baseline_r = reward.mean(dim = 1 )
            baseline_r = baseline_r.repeat_interleave(seg)
            baseline_r = baseline_r.view(B, 1).detach()
            reward = preds[:,0]
            lam = reward-baseline_r
            #print(lam.shape)
            sigma = lam ** 2
            sigma = sigma.view(mini_number, seg)
            sigma = sigma.mean(dim = 1)  # sigma.shape = [2]
            sigma = torch.sqrt(sigma+delta)
            sigma = sigma.repeat_interleave(seg)
            sigma = sigma.view(B, 1).detach()
            #print(sigma.shape)
            advantage = lam / (sigma) # delta = 1e-8
            advantage = advantage.view(B, 1, 1)

            #for i in range(mini):
                #for j in range(B/mini):
                

            #baseline_r = torch.stack(reward_list).mean()
            #baseline_r = baseline_r.expand(B)
            #baseline_r = baseline_r.view(B, 1)
            ...
            for random_t in range(cfg.total_num_steps):
                ...
                #reward_now = reward_list[random_t]
                '''
                baseline_r = baseline_r_list[random_t]
                lam = reward_now-baseline_r
                #print(lam.shape)
                sigma = lam ** 2
                sigma = sigma.view(mini_number, seg)
                sigma = sigma.mean(dim = 1)  # sigma.shape = [2]
                ...
                #print(sigma.shape)
                advantage = lam / (sigma + delta) # delta = 1e-8
                advantage = advantage.view(B, 1, 1)
                '''
                ...
                ratio = p_x0 / p_x0_old
                clipped_ratio = torch.clamp(ratio, 1 - clip, 1 + clip) #eps = 1e-5
...
if __name__ == "__main__":
    base_path = "/home/ubuntu"  #outputs_gosai/2025.12.27/080231/checkpoints/best.ckpt  alpha0.001_accum4_bsz1_truncate10_temp1.0_clip1.0_test_20260301_172219
    ckpt_path = os.path.join(base_path, 'mdlm/reward_bp_results_final/alpha0.001_accum4_bsz1_truncate10_temp1.0_clip1.0_test_20260302_173249/model_1399.ckpt') 
